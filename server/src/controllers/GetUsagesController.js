...
//water mock data init
async getLastMinuteInSecondsWater (req, res) {
    try {
        const body = req.body
        const user = await User.findOne({
            where: {
    ...
    res.status(500).send({
        error: 'An error has occured getting mock electric and mock water seconds device'
    })
}
},
async getLastHourInMinutesWater (req, res) {
    try {
        const body = req.body
        const user = await User.findOne({
            where: {
        ...
        res.status(500).send({
        error: 'An error has occured getting mock electric and mock water minutes device'
        })
    }
},
}
